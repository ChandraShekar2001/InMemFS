...
private:
    void worker_function() {
        while (true) {
            Task task{};
            {
                ...
                // this cv ensures that if the tasks are complete then block the thread until another tasks come
    ...
    bool create_file(const string &filename) {
        //take a lock of the critical section
        ...
        files[filename] = File();
        //the lock is unlocked automatically when it exits the scope
        return true;
    }
