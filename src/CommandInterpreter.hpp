...
private:
    ...
    static bool validateCreate(const string &input, ValidationResult &result, const vector<string> &tokens) {
        ...
        //if it is a simple file creation then we dont use threads to create them
        ...
        if (regex_match(input, matches, createMultiple)) {
            ...
            for (size_t i = 3; i < tokens.size(); ++i) {
                //if the name is not valid then throw error
            ...
            //check if we got the same number of files as given in the cli
...
public:
    ...
    void process(const string &line) {
        try {
            ...
            //the first token signifies the operation
            // for ex.: create a.txt, here command = create
            const string &command = tokens[0];
            if (command == "create") {
                ValidationResult result;
                //check for any possible errors and throw error in this stage
